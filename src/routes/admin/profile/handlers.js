...
internals.index = async function (req, reply) {
  ...
  return reply.view("admin/profile.html", {
    credentials,
  });
};
internals.update = async function (req, reply) {
  ...
  if (!userUpdate) {
    return reply.redirect("/admin/profile?message=Error to udpdate&alert=error");
  }
  return reply.redirect("/admin/profile?message=Successfully update&alert=success");
};
